...
final class NoteRepository
{
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM notes WHERE user_id = :id AND uuid = :uuid LIMIT 1');
    ...
    public function create(int $userId, string $uuid, string $encryptedPayload, string $nonce, bool $favorite): int
    {
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO notes (user_id, uuid, encrypted_payload, payload_nonce, favorite, archived, created_at, updated_at)
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE notes SET encrypted_payload = :enc, payload_nonce = :nonce, favorite = :fav, updated_at = :now
    ...
    {
        $stmt = $this->db->prepare(
            'UPDATE notes SET archived = :archived, updated_at = :now WHERE user_id = :user_id AND uuid = :uuid'
        );
        $stmt->execute([
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM notes WHERE user_id = :user_id AND uuid = :uuid');
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM notes WHERE user_id = :id');
        $stmt->execute([':id' => $userId]);
    }

    public function existsByUuid(int $userId, string $uuid): bool
    {
        $stmt = $this->db->prepare('SELECT 1 FROM notes WHERE user_id = :id AND uuid = :uuid LIMIT 1');
        $stmt->execute([':id' => $userId, ':uuid' => $uuid]);
